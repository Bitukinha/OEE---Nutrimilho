...
import { calcularDisponibilidadeComParadas, calcularTempoPlanejadoTurnoMinutos, formatDateISO } from '@/lib/dateUtils';
...
export const useCreateRegistroProducao = () => {
  ...
  return useMutation({
    mutationFn: async (registro: RegistroProducaoInput) => {
      ...
      const tempoPlanejado = turnoData
        ? calcularTempoPlanejadoTurnoMinutos(turnoData.hora_inicio, turnoData.hora_fim)
        : Math.max(1, registro.tempo_planejado);
      const tempoReal = Math.min(tempoPlanejado, Math.max(0, registro.tempo_real || tempoPlanejado));

      // Disponibilidade: tempo_real / tempo_planejado (%), cap 100
      const disponibilidade = tempoPlanejado > 0 ? Math.min(100, (tempoReal / tempoPlanejado) * 100) : 100;
